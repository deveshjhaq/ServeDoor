...
if(isset($_POST['submit'])){
	...
	$restaurant_id=get_safe_value($_POST['restaurant_id']);
	...
	/* ---- Basic validations ---- */
	if($restaurant_id==''){
		$msg="Please select Restaurant";
	}
	if($msg==''){
		// restaurant must be valid+active (optional but recommended)
		$rs_chk = mysqli_query($con,"select id from restaurants where id='$restaurant_id' and status=1");
		if(mysqli_num_rows($rs_chk)==0){
			$msg="Please select a valid Restaurant";
		}
	}

	/* ---- Duplicate dish name per restaurant ---- */
	if($msg==''){
		if($id==''){
			$sql="select * from dish where dish='$dish' and restaurant_id='$restaurant_id'";
		}else{
			$sql="select * from dish where dish='$dish' and restaurant_id='$restaurant_id' and id!='$id'";
		}
		if(mysqli_num_rows(mysqli_query($con,$sql))>0){
			$msg="Dish already added for this restaurant";
		}
	}

	/* ---- Proceed if no validation errors ---- */
	if($msg==''){
		$type_mime = isset($_FILES['image']['type']) ? $_FILES['image']['type'] : '';

		if($id==''){ // INSERT
			// image required on create
			if($type_mime!='image/jpeg' && $type_mime!='image/png'){
				$image_error="Invalid image format";
			}else{
				$image=rand(111111111,999999999).'_'.$_FILES['image']['name'];
				move_uploaded_file($_FILES['image']['tmp_name'],SERVER_DISH_IMAGE.$image);

				mysqli_query($con,"insert into dish(category_id,restaurant_id,dish,dish_detail,status,added_on,image,type) values('$category_id','$restaurant_id','$dish','$dish_detail',1,'$added_on','$image','$food_type')");
			...
			if($image_error==''){
				$sql="update dish set category_id='$category_id', restaurant_id='$restaurant_id', dish='$dish' , dish_detail='$dish_detail', type='$food_type' $image_condition where id='$id'";
				mysqli_query($con,$sql);

				$attributeArr=$_POST['attribute'];
				$priceArr=$_POST['price'];
				$statusArr=$_POST['status'];
				$dishDetailsIdArr= isset($_POST['dish_details_id']) ? $_POST['dish_details_id'] : [];
...
$res_restaurants=mysqli_query($con,"select id,name from restaurants where status='1' order by name asc");
                  ...
                  <form class="forms-sample" method="post" enctype="multipart/form-data">
					...
					<div class="form-group">
                      ...
                      <select class="form-control" name="restaurant_id" required>
						...
						<?php
						while($row_rs=mysqli_fetch_assoc($res_restaurants)){
							$sel = ($row_rs['id']==$restaurant_id) ? "selected" : "";
							echo "<option value='".$row_rs['id']."' $sel>".$row_rs['name']."</option>";
						}
						?>
					  </select>
                    </div>
					...
					<div class="form-group">
					  ...
					  <div class="error mt8"><?php echo ($msg && strpos($msg,'Dish already')!==false)?$msg:''?></div>
                    </div>
