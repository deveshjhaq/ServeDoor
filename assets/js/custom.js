...
function add_to_cart(id,type){
	...
	if(typeof attr === 'undefined'){
		swal("Error", "Please select an option before adding to cart.", "error");
		return;
	}
	...
	jQuery.ajax({
		...
		success:function(result){
            ...
            jQuery('#shopTotal').html(data.totalPrice + ' Rs');
			
			var cartHTML = '';
			jQuery.each(data.cartArr, function(key, item) {
				var itemTotalPrice = item.qty * item.price;
				cartHTML += '<li class="flex gap-3 py-2 border-b theme-border last:border-0" id="attr_' + key + '">' +
                    '<img src="' + SITE_DISH_IMAGE + item.image + '" class="w-12 h-12 rounded object-cover" alt="">' +
                    '<div class="flex-1">' +
                    '<div class="text-sm font-medium">' + item.dish + '</div>' +
                    '<div class="text-xs theme-muted">Qty: ' + item.qty + '</div>' +
                    '<div class="text-sm">' + itemTotalPrice + ' Rs</div>' +
                    '</div>' +
                    '<button onclick="delete_cart(\'' + key + '\')" class="text-red-400 hover:text-red-300">' +
                    '<span class="material-symbols-outlined text-base">close</span>' +
                    '</button>' +
                    '</li>';
			});
			jQuery('#cart_ul').html(cartHTML);
		}
	});
}
